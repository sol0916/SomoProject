...
public class MemberController extends HttpServlet {
	...
	protected void doAction(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(command.equals("/member/member_join.mem")) {
			...
			//비밀번호 확인 (회원 정보 수정, 삭제 시)
		} else if(command.equals("/member/member_pwcheck.mem")) {

			MemberServiceImpl si = new MemberServiceImpl();
			int result = si.pwCheck(request, response);
			...
			if(result==1) {
				...
				if(check.equals("1")) {
					...
					out.println("location.href='member_modify.mem';");
					out.println("</script>");
					...
					out.println("<script>");
					out.println("alert('확인되었습니다');");
					out.println("location.href='member_delete.mem';");
					out.println("</script>");
